 ...
 *
 * <p>
 * The native library {@code libwebp_animdecoder} (custom compiled for this from the libwebp project) needs to be made available before any decoding attempts.
 * For supported platforms they are packed in the JAR and can be extracted using the {@link #init()} function, which must be run before any of the decode or
 * test functions. When not using {@link #init()} JNA will look for the library in various places (consult the JNA docs for details).
 *
 * <p>
 * The functions using the native libraries may throw an {@code UnsatisfiedLinkError}. Since this is an error it is recommended to catch it explicitly instead
 * of catching {@code Error} or {@code Throwable}.
 *
 * @author tduva
 */
public class WebPDecoder {
	...
	/**
	 * This function is intended to be used before decoding is attempted (or {@link #test()} is used). Extracts the platform dependent library from the JAR to a
	 * temp folder chosen by JNA.
	 *
	 * <p>
	 * When this function is not used the regular native library discovery mechanism of JNA will be used.
	 *
	 * @throws IOException
	 *             When extracting a library fails, in which case it may not be possible to decode images
	 * @see #init(boolean)
	 */
	 ...
	 *
	 * @param nextToJar
	 *            Check if the native library can be found next to the JAR (same directory) this class is contained in, otherwise extract from the JAR as normal
	 * @throws IOException
	 *             When extracting a library fails, in which case it may not be possible to decode images
	 */
	...
	private static synchronized LibWebP lib() {
		if (libWebPInstance == null) {
			libWebPInstance = Native.loadLibrary(libPath != null ? libPath.toString() : LIB_NAME, LibWebP.class);
	...
	/**
	 * On Windows JNA can't remove the temp file while it's still in use, so an ".x" file with the same name is created so it can be deleted on the next start.
	 * When using the extract function directly that is not done by JNA, so do it here.
	 *
	 * @param path
	 */
	...
	// ==========================
	/**
	 * Same as {@link #testEx()} but instead of throwing an exception it only return {@code true} or {@code false}.
	 *
	...
	/**
	 * Decode a test image included in the JAR to check if the native libraries are properly loaded and if everything seems to work.
	 *
	 * @throws IOException
	 *             If loading the test image fails
	 * @throws WebPDecoderException
	 *             If the decoder encounters an error
	 * @throws UnsatisfiedLinkError
	 *             When there was an issue loading the native libraries (note that this is an error, not an exception)
	 */
	 ...
	 * @throws IOException
	 *             When loading the data from the url fails
	 * @throws WebPDecoderException
	 *             When the decoder encounters an issue (e.g. if it's not a valid WebP file)
	 * @throws UnsatisfiedLinkError
	 *             When there was an issue loading the native libraries (note that this is an error, not an exception)
	 */
	...
	// ==========================
	/**
	 * A decoded image containing the individual frames (for static images just one) and some meta info.
	 */
	...
	public static class WebPImageFrame {
		...
		/**
		 * Counted from the start of the animation until when to show the frame (in ms).
		 */
	...
	private interface LibWebP extends Library {
		...
		/*
		 * [webp/types.h] // Allocates 'size' bytes of memory. Returns NULL upon error. Memory // must be deallocated by calling WebPFree(). This function is
		 * made available // by the core 'libwebp' library. WEBP_EXTERN void* WebPMalloc(size_t size);
		 */
		public Pointer WebPMalloc(int size);

		/*
		 * [webp/types.h] // Releases memory returned by the WebPDecode*() functions (from decode.h). WEBP_EXTERN void WebPFree(void* ptr);
		 */
		...
		/*
		 * [webp/demux.h] // Internal, version-checked, entry point. WEBP_EXTERN WebPAnimDecoder* WebPAnimDecoderNewInternal( const WebPData*, const
		 * WebPAnimDecoderOptions*, int); // Creates and initializes a WebPAnimDecoder object. // Parameters: // webp_data - (in) WebP bitstream. This should
		 * remain unchanged during the // lifetime of the output WebPAnimDecoder object. // dec_options - (in) decoding options. Can be passed NULL to choose //
		 * reasonable defaults (in particular, color mode MODE_RGBA // will be picked). // Returns: // A pointer to the newly created WebPAnimDecoder object, or
		 * NULL in case of // parsing error, invalid option or memory error. static WEBP_INLINE WebPAnimDecoder* WebPAnimDecoderNew( const WebPData* webp_data,
		 * const WebPAnimDecoderOptions* dec_options) { return WebPAnimDecoderNewInternal(webp_data, dec_options, WEBP_DEMUX_ABI_VERSION); }
		 */
		public Pointer WebPAnimDecoderNewInternal(WebPData webp_data, Structure dec_options, int version);

		/*
		 * [webp/mux_types.h] // Data type used to describe 'raw' data, e.g., chunk data // (ICC profile, metadata) and WebP compressed image data. // 'bytes'
		 * memory must be allocated using WebPMalloc() and such. struct WebPData { const uint8_t* bytes; size_t size; };
		 */
		public static class WebPData extends Structure {
			...
			protected List<String> getFieldOrder() {
				return Arrays.asList("bytes", "length");
			}
		}

		/*
		 * [webp/demux.h] // Get global information about the animation. // Parameters: // dec - (in) decoder instance to get information from. // info - (out)
		 * global information fetched from the animation. // Returns: // True on success. WEBP_EXTERN int WebPAnimDecoderGetInfo(const WebPAnimDecoder* dec,
		 * WebPAnimInfo* info);
		 */
		public int WebPAnimDecoderGetInfo(Pointer dec, WebPAnimInfo info);

		/*
		 * [webp/demux.h] // Global information about the animation.. struct WebPAnimInfo { uint32_t canvas_width; uint32_t canvas_height; uint32_t loop_count;
		 * uint32_t bgcolor; uint32_t frame_count; uint32_t pad[4]; // padding for later use };
		 */
		public static class WebPAnimInfo extends Structure {
			...
			@Override
			protected List<String> getFieldOrder() {
				return Arrays.asList("canvas_width", "canvas_height", "loop_count", "bgcolor", "frame_count", "pad");
			}
		}

		/*
		 * [webp/demux.h] // Check if there are more frames left to decode. // Parameters: // dec - (in) decoder instance to be checked. // Returns: // True if
		 * 'dec' is not NULL and some frames are yet to be decoded. // Otherwise, returns false. WEBP_EXTERN int WebPAnimDecoderHasMoreFrames(const
		 * WebPAnimDecoder* dec);
		 */
		public int WebPAnimDecoderHasMoreFrames(Pointer dec);

		/*
		 * [webp/demux.h] // Fetch the next frame from 'dec' based on options supplied to // WebPAnimDecoderNew(). This will be a fully reconstructed canvas of
		 * size // 'canvasWidth * 4 * canvasHeight', and not just the frame sub-rectangle. The // returned buffer 'buf' is valid only until the next call to //
		 * WebPAnimDecoderGetNext(), WebPAnimDecoderReset() or WebPAnimDecoderDelete(). // Parameters: // dec - (in/out) decoder instance from which the next
		 * frame is to be fetched. // buf - (out) decoded frame. // timestamp - (out) timestamp of the frame in milliseconds. // Returns: // False if any of the
		 * arguments are NULL, or if there is a parsing or // decoding error, or if there are no more frames. Otherwise, returns true. WEBP_EXTERN int
		 * WebPAnimDecoderGetNext(WebPAnimDecoder* dec, uint8_t** buf, int* timestamp);
		 */
		public int WebPAnimDecoderGetNext(Pointer dec, PointerByReference buf, IntByReference timestamp);

		/*
		 * [webp/demux.h] // Deletes the WebPAnimDecoder object. // Parameters: // dec - (in/out) decoder instance to be deleted WEBP_EXTERN void
